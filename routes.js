...
const models = require('/usr/src/app/code/models')
  ...
  models.Profile.findOne({
    ...
    if (!profile) {
      return res
        .status(404)
        .status(
          'Profile! Not found.<br><a href="/">Tillbaka till startsidan</a>'
        )
    }
      ...
      models.Ration.findOne({
        ...
        if (!ration) {
          return res
            .status(404)
            .status(
              'Ration Not found.<br><a href="/calculate">Tillbaka till beräkningssidan</a>'
            )
        }
        ...
        ration
          ...
          .catch((error) => {
            if (error) res.status(500).status('Something went wrong')
          })
      })
    } else {
...
router.post('/profile/add', (req, res) => {
  models.Profile.findOne({
    ...
    .then((name) => {
      ...
      if (name) {
        return res.status(500, 'A profile with that name already exists')
      }
...
router.post('/profile/edit', (req, res) => {
  models.Profile.findOne({
    ...
    profile.profileBorn = req.body.born
    profile.profileGender = req.body.gender
    profile.profileWeight = req.body.weight
    profile.profileType = req.body.type
    profile.profileLook = req.body.look
    profile.profileWalk = req.body.walk
    profile.profileTrotCanter = req.body.trotCanter
...
// Change profile page.
router.get('/profile/:profile/edit', (req, res) => {
  models.Profile.findOne({
    where: { profileName: req.params.profile },
  }).then((profile) => {
    if (!profile) {
